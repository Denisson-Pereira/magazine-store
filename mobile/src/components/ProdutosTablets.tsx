...
export const ProdutosTablets = () => {
  const [dados, setDados] = useState<IProdutos[]>([]);
  const { navigate } = useNavigate();
  const { favorites, toggleFavorite } = useFavorites();

  const [tablet, setTablet] = useState<IProdutos[]>([])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDados(response);
        setTablet(dados.filter(item => item.categoria == 'tablet'))
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tituloCelulares}>
        <Text style={styles.textoTituloSecao}>🎉 Os tablets que estão conquistando o mercado!</Text>
      </View>
